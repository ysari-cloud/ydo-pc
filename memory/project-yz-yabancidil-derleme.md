...
- 🔄 **YENİ İŞ MODELİ (kullanıcı kararı, 2026-06-24 — TAM YEREL):** Proje **tamamen yerele** taşındı; bulut günlük döngüden çıktı. **DÜZELTME:** "Cowork" yerelde ÇALIŞIYORDU ve TÜM yerel dosyalara/PDF'lere erişimi vardı; eksik olan tek şey **buluta/origin'e (git push) erişimdi** — yapıştırma derdi bundandı. Çözüm: her şey yerel Claude Code'da, tek klasör (`C:\ydo-pc`, OneDrive DIŞI). Cloud, kullanıcıya komple güncel projeyi `.zip` olarak SendUserFile ile gönderdi (origin `0e443c8`); kullanıcı açıp PDF'leri `_pdf\`'e koyacak. Bundan sonra bulut çağrılmaz (gerekirse on-call). GitHub origin yalnız yedek/geçmiş olarak durur. Navigasyon: `00-NAVIGASYON.md`.
- 🗑️ **DEPO TEMİZLİĞİ (kullanıcı kararı, 2026-07-11):** `projects/yz-yabancidil-derleme/` klasörünün TAMAMI (138 dosya) hem `claude/cleanup-local-files-int6g2` hem de default dal `claude/youthful-franklin-wszezx`'ten silindi — yerelde (`C:\ydo-pc`) son sürümler zaten mevcut olduğu için bulut kopyası karışıklığa yol açıyordu. `.gitignore` de bu klasöre atıf yaptığı için kaldırıldı. Dosyalar git geçmişinde korunuyor (commit `05c321c` öncesi), gerekirse `git show 35f8993:projects/yz-yabancidil-derleme/<dosya>` ile geri getirilebilir. **Bu proje artık bu depoda dosya barındırmıyor** — sadece bu hafıza notu duruyor.

## 🔖 DEVAM NOKTASI

### ⏭️ SIRADAKİ (YENİ OTURUM BURADAN DEVAM) — 2026-06-24 (KODLAMA KİLİTLENDİ → SENTEZ)

> 🎯 **DURUM:** **AS4–AS5 BULGULAR DÜZYAZISI HAZIR.** Görev-7+8 (kodlama kilitli) → Görev-9 (cowork sentez düzyazısı yazdı, cloud denetledi+düzeltti+push etti) → **`26-bulgular-AS4-AS5-taslak.md`** (İngilizce, 10 tema, 12 İz alıntısı, iki zorunlu kural yerinde). Cloud düzeltmeleri: AS4-T5 dil sayısı five→four; iskelet yol ref; bakım uyarısı çözüldü.
> **TEK SONRAKİ ADIM (öncelik):** **Bütünlük görevi — 25 dahil çalışmanın DOI/künye doğrulaması** (cloud yapar, web erişimi var). v3 bütünlük notu: şu an yalnız 12 referans doğrulanmış; 25 dahil çalışma henüz doğrulanmış kaynakçada DEĞİL → kaynakça kilidi için kritik. Sonra: (a) v4 taslağına `26`'yı işleme (cowork); (b) Tartışma+Giriş güncelleme (C4↔C6 gerilimi, "what this paper adds" 3 çengel).
...
- **✅ 6 ANLAŞMAZLIK TAM-METİNLE UZLAŞTI (consensus coding):** #6 C1 a→**c** · #35 C1 c→**c′** · #29 C3 4→**1** (otonom sistem, öğretmen kör-değerlendirici) · #34 C3 4→**3** (anket-beyanı, gözlenmedi) · #6&#19 C6 **çift kutup** · #34 üretici=in-service. İşlendi: `kol2/27` (adj.logu) + `kol2/30` (uzlaşı tablosu).
- **✅ codebook v1.2 KİLİTLENDİ (`04`):** C1 a/c/c′ ayracı (kim üretiyor × yinelemeli mi) · C2 c-vs-d (öğrenci eş-yaratım→d) · C3 1–5 somut çapa (otonom son-kullanıcı=1, anket-beyanı=3 tavan, gözlenen vs beyan) · C6 risk+pozitif çift kutup.
- **KAVRAMSAL TEYİT (kullanıcı):** Üretici kimliği (in-service/pre-service/öğrenci) = yalnız C2 b/b⁺/c **dağılım etiketi**, dahil/amaç FİLTRESİ DEĞİL. 6 makalenin **hepsi İÇERİDE** (N=25'in parçası).
- **METODOLOJİK ÇERÇEVE:** "İlk-tur boyut-uyum orta (~66%) → 6 anlaşmazlık consensus coding + tam-metinle çözüldü → kurallar v1.2'de rafine → tam set yeniden kodlandı." (Two Years/Griche emsallerinden güçlü.) *(κ alt-kümesi 6/25≈24% yeterli; istenirse v1.2 sonrası ~12'ye genişletilip yeniden-κ — opsiyonel.)*
- **Son commit:** `0292bf3` (Görev-7 talimatı). Çalışma ağacı temiz, origin = HEAD.
