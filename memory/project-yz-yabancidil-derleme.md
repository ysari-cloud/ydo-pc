...
1. **Komşu SSCI derleme YÖNTEM-ANALİZİ (cloud işi):** Bunlar bizim 25 korpus DEĞİL, **başkalarının derlemeleri** (yazım şablonu). **İkincil PDF'ler repoda YOK (gitignore) → yeni oturumda cloud'a YENİDEN YÜKLE.** Liste: **(a) elimizde, re-upload:** Deng & Jamaludin 2026 (SAGE Open), Griche & Bennis 2026 (TLTL). **(b) İNDİR (yeni):** Two Years of Innovation (C&E:AI, `10.1016/j.caeai.2025.100445`) [en yakın SR] · 1 bibliyometrik (`10.1007/s10639-024-12937-z` AI-in-EFL VEYA Scientific Mapping GenAI-EFL) · opsiyonel ILE/CALL'da 1 derleme (ev-stili). → Cloud `komsu-yontem-analizi.md` (yapı/PRISMA/kalite/sentez/çerçeve + ayrışma haritası) = yazım iskeleti. **Tüm PDF disposition: `kol2/envanter-tum-pdfler.md`.**
