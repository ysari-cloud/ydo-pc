...
CONV_LEX = ['chatbot','conversational agent','conversation','dialogue','speaking','oral',
            'pronunciation','intelligent tutoring systems','tutoring','interaction','dialog',
            'spoken','virtual assistant','voice','listening','tutor']
PROD_LEX = ['material','materials development','content creation','content generation',
            'instructional design','courseware','lesson plan','generative','automatic',
            'automated','test generation','item generation','question generation','tool',
            'development','design','creation','authoring','prompt engineering','writing generation']
def slice_of(y):
...
def lex_share(ds):
    ...
    for name,_ in SLICES:
        ...
        n=len(recs) or 1
        conv=sum(1 for r in recs if any(t in text_blob(r) for t in CONV_LEX))
        prod=sum(1 for r in recs if any(t in text_blob(r) for t in PROD_LEX))
